...
def _load_no_yolo_sidecar(video_path: str):
    """
    Reads ZM_ALARM_QUEUE/<camid>-<eventid>.txt (JSON) created by poll_zm_for_events.py.
    Returns list of zones: [{"Units":..., "Coords":...}, ...]
    """
    ...
    try:
        ...
        # keep only zones named no_yolo (defensive)
        out = []
        for z in zones:
            name = str(z.get("Name", ""))
            if name.lower() == "no_yolo" and z.get("Coords"):
                out.append(z)
        return out
    except Exception as e:
        printLog(f"⚠️ Failed to read sidecar {sidecar_path}: {e}")
        return []
...
def _coords_to_poly(zone, frame_w, frame_h):
    pts = _parse_coords(zone["Coords"])
    units = str(zone.get("Units", "")).lower()

    # If units looks like percent, treat as 0..100 percentages
    # (fallback heuristic if Units is missing/odd)
    if "percent" in units or (pts and max(p[0] for p in pts) <= 100 and max(p[1] for p in pts) <= 100):
        pts = [(p[0] * frame_w / 100.0, p[1] * frame_h / 100.0) for p in pts]
...
def process_video(video_path, model, camid, event_id):
    ...
    # ----------------------------
    # Load no_yolo polygons (once)
    # ----------------------------
    ignore_polys = []
    try:
        zones = _load_no_yolo_sidecar(video_path)
        if zones:
            # read one frame to get dimensions then rewind
            ret0, frame0 = cap.read()
            if ret0:
                h0, w0 = frame0.shape[:2]
                cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                ignore_polys = [_coords_to_poly(z, w0, h0) for z in zones]
            else:
                cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
    except Exception as e:
        printLog(f"⚠️ no_yolo sidecar parse failed for {video_path}: {e}")
        try:
            cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
        except Exception:
            pass
    ...
    while cap.isOpened():
        ...
        if frame_count % frame_interval == 0:
            ...
            for obj_name, data in detected_objects.items():
                ...
                # Skip detections whose box-center is inside any no_yolo polygon
                if ignore_polys:
                    cx = x + w / 2.0
                    cy = y + h / 2.0
                    if any(_point_in_poly(cx, cy, poly) for poly in ignore_polys):
                        continue
...
#                printLog(f"🗑️ Deleted {video_path}")
            ...
            # Delete the sidecar json (if present)
            sidecar_path = os.path.splitext(video_path)[0] + ".json"
            try:
                if os.path.exists(sidecar_path):
                    os.remove(sidecar_path)
#                    printLog(f"🗑️ Deleted {sidecar_path}")
            except Exception as del_err:
                printLog(f"❌ Failed to delete {sidecar_path}: {del_err}", file=sys.stderr)
