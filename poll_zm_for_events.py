...
# ==========================
# Zone / Privacy Mask Helpers (for YOLO sidecar)
# ==========================

NO_YOLO_ZONE_NAME = "no_yolo"  # zones with this name are treated as privacy masks for YOLO

def get_no_yolo_zones(monitor_id: str):
    """Return list of zones for this monitor where Name == 'no_yolo' (case-insensitive)."""
    login_bauth()
    if not access_token:
        return []

    url = f"{ZM_HOST}/zm/api/zones.json?token={access_token}"
    auth = HTTPBasicAuth(BAUTH_USER, BAUTH_PWD)
    try:
        r = requests.get(url, auth=auth, verify=False, timeout=10)
        if r.status_code != 200:
            printLog(f"⚠️ Failed to retrieve zones: {r.status_code} - {r.text[:200]}")
            return []
        data = r.json()
    except Exception as e:
        printLog(f"⚠️ Error retrieving zones: {e}")
        return []
    ...
    for z in zones:
        zone = z.get("Zone", z)
        if str(zone.get("MonitorId")) != str(monitor_id):
            continue
        name = str(zone.get("Name", ""))
        if name.lower() != NO_YOLO_ZONE_NAME:
            continue
        ...
        out.append({
            "Name": name,
            "MonitorId": str(zone.get("MonitorId")),
            "Units": zone.get("Units"),
            "Coords": coords,
        })
    return out

def write_no_yolo_sidecar(video_path: str, monitor_id: str, event_id: str = ""):
    """Write <video_basename>.txt JSON sidecar containing 'no_yolo' zone polygons."""
    try:
        zones = get_no_yolo_zones(monitor_id)
        ...
        printLog(f"⚠️ Failed to write no_yolo sidecar for {video_path}: {e}")
...
#            printLog(f"Downloaded event {event_id} video to {output_path}")
            # Write privacy-zone sidecar for YOLO (zones named 'no_yolo')
            write_no_yolo_sidecar(output_path, monitor_id, event_id)
            return True
        else:
