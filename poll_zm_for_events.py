...
def login_bauth():
    ...
    response = requests.post(url, auth=HTTPBasicAuth(BAUTH_USER, BAUTH_PWD),
                             data={'user': ZM_USER, 'pass': ZM_PASS, 'stateful': '1'}, verify=False)
    ...
    if response.status_code == 200:
        ...
        printLog(f"❌ Login failed: {response.text}")
        return None
